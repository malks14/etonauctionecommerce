...
const Post = ({ match }) => {
  ...
  return (
    ...
    >
      <div className="link-wrapper"  style={{maxWidth: "400px"}}>
        <Link to="/sample">Back to Home page</Link>
      </div>
      <div className="post" style={{maxWidth: "400px"}}>
        <img className="post__img" src={imgSrc} alt={title}  style={{maxWidth: "400px"}}/>
  ...
  return (
    <motion.div className="post-preview" variants={postPreviewVariants}>
      ...
      <div className="post-preview__text">
        ...
        <p>{description}</p>
        <Link href={`/post/${id}`} passHref>Learn more</Link>
      </div>
    </motion.div>
  );
};
